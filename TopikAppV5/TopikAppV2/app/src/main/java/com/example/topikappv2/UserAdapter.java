...
public class UserAdapter extends BaseAdapter{
    ...
    public void deleteItem2(int problem_number) {
        int i = 1;
        while (i <= mUserSet.size()){
            if(mUserSet.get(mUserSet.size()).getArranged_num() == String.valueOf(problem_number)){
                mUserSet.remove(mUserSet.get(mUserSet.size()));
            }
            i +=1;
        }
    }
...
//                getP_answer(position).toString());
        return new UserSet(getProblem_id(position).toString(),getArranged_num(position).toString(),getNumber(position).toString(),getU_answer(position).toString(),
                getP_answer(position).toString(),getProb_set(position).toString(),getScore(position).toString(), (Integer) getTime_Stamp(position), (Integer) getChange(position),
                getSection(position).toString());
    }

    public Object getProblem_id(int position) { return mUserSet.get(position).getProblem_id();}
    ...
    public Object getTime_Stamp(int position) {return mUserSet.get(position).getTime_Stamp();}
    public Object getChange(int position) {return mUserSet.get(position).getChange();}
    public Object getSection(int position) {return mUserSet.get(position).getSection();}
    ...
    public View getView(int position, View convertView, ViewGroup parent){
        ...
        resultView.setTime_stamp(mUserSet.get(position).getTime_Stamp());
        resultView.setChange(mUserSet.get(position).getChange());
